...
using System.Text;
using ABCBankFrontendOriginal.Servicios;
...
namespace ABCBankFrontendOriginal.Servicios
{
    public class Servicio_Api : IServicio_Api
    {
        ...
        public async Task<Contacto> filtrar(string buscarNombre, string buscarDireccion)
        {
            var nombres = buscarNombre;
            var direccion = buscarDireccion;
            Contacto objeto = new Contacto();

            var cliente = new HttpClient();
            cliente.BaseAddress = new Uri(_baseurl);
            var response = await cliente.GetAsync($"api/Contacto/id:int nombres:string direccion:string?nombres={nombres}&direccion={direccion}");
